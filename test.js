...
this.subscribers = {};
 this.history = {};
   ...
   MongoClient.connect(url, function(err, db) {
     if (err) throw err;
      ...
      dbo.collection("sublist").find({}).toArray(function(err, result) {
        if (err) throw err;
        if(result.length==0)
        {
 }
   ...
   console.log(result);
   if(result.sub!=null)
   this.subscribers = result.sub;
   if(result.sublist !=null)
   this.subscribersList = result.sublist;
 ...
 MongoClient.connect(url, function(err, db) {
   if (err) throw err;
   var dbo = db.db("pubsub");
   var newValues = { sublist : a , sub : b , history:c};
   console.log("hey" + newValues);
     dbo.collection("sublist").updateOne({}, newValues, function(err, res) {
